...
private:
    ...
    double findMedianUtil( int A[], int N, int B[], int M )
    {
        ...
        if (N == 1)
        {
            ...
            // A[] = {1}, B[] = {5, 8, 10, 20, 30}
            if (M & 1)
                return MO2( B[(M-1)/2], MO3(A[0], B[(M-1)/2 - 1], B[(M-1)/2 + 1]) );
     
            // Case 3: If the larger array has even number of element,
            // then median will be one of the following 3 elements
            // ... The middle two elements of larger array
            // ... The only element of smaller array
            return MO3( B[(M-1)/2], B[(M-1)/2 + 1], A[0] );
        }
        ...
        int idxB = ( M - 1 ) / 2;
         ...
         /* if A[idxA] <= B[idxB], then median must exist in
            A[idxA....] and B[....idxB] */
        if (A[idxA] <= B[idxB] )
          return findMedianUtil(A + idxA, N - idxA, B, M - idxA );
     
        /* if A[idxA] > B[idxB], then median must exist in
           A[...idxA] and B[idxB....] */
        return findMedianUtil(A, N - idxA, B + idxA, M - idxA );
    }
...
int main() {
    {
        vector<int> a = {}, b = {};
        std::cout << Solution().findMedianSortedArrays(a, b) << "\n";  // -1  invalid
    }
    {
        vector<int> a = {}, b = {1};
        std::cout << Solution().findMedianSortedArrays(a, b) << "\n";  // 1
    ...
    {
        vector<int> a = {6}, b = {2,3,4,5};
        std::cout << Solution().findMedianSortedArrays(a, b) << "\n";  // 4
    }
    {
        vector<int> a = {1,3,5}, b = {2,4};
        std::cout << Solution().findMedianSortedArrays(a, b) << "\n";  // 3
    }
    {
